...
export const IdcardUtils = {getInfo(idcard: string): {
  adCode: string
  gender: boolean
  birthday: timestamp
} | undefined {
  ...
  return Regexes.CHINA_ID_CARD.test(trimIdcard)
    ? {adCode: trimIdcard.slice(0, 6), gender: Number(trimIdcard.slice(16, 17)) % 2 === 0, birthday: birthday satisfies timestamp}
    : void 0
}}
