...
const effectVModels = computed(() => {
  ...
  _allFields.forEach(f => {
    ...
    fieldProps[label] = f.value // 添加值绑定

    fieldProps[`onUpdate:${label}`] = (v: dynamic) => f.setValue(v) // 添加事件监听
  })

  const commonEvents = {'onBlur': (e: Event) => _allFields[0]?.handleBlur(e, true), 'onReset': () => _allFields[0]?.handleReset, 'onUpdate:errorMessages': (v: string | string[]) => _allFields.forEach(f => f.setErrors(maybeArray(v)))} // 添加通用事件

  return {...baseProps, ...fieldProps, ...commonEvents} // 合并所有属性
})
</script>
