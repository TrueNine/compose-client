...
const rule: Rule.RuleModule = {
  ...
  create(context) {
    ...
    function normalizeConditionText(text: string): string {
      return text /* 将多行条件压缩成单行，处理 || 和 && 分隔的情况 */
        .split('\n')
        .map(line => line.trim())
        .join(' ')
        .replaceAll(/\s+/g, ' ')
    ...
    function canConditionBeSingleLine(test: Rule.Node): boolean {
      ...
      return condText.length < MAX_LINE_LENGTH - 40 /* 条件本身不能太长 */
    }
    ...
    return {
      IfStatement(node) {
        const {parent} = node
        if (parent?.type === 'IfStatement' && 'alternate' in parent && parent.alternate === node) return /* Skip if this is part of an else-if chain (will be handled by the root if) */

        if (!isAlreadySingleLine(node) && canConvertToSingleLine(node)) { /* First, try to convert the entire chain */
          context.report({
            node,
        ...
        const {conditions, finalElse} = collectIfChain(node) /* If entire chain can't be converted, try to simplify individual branches */

        const branchSimplifiable: boolean[] = [] /* Check which branches can be simplified */
        for (const cond of conditions) {
        ...
        const hasSimplifiableBranch = branchSimplifiable.some(Boolean) || finalElseSimplifiable /* Check if there's anything to simplify */
        if (!hasSimplifiableBranch) return

        context.report({
          node,
          messageId: 'preferSingleLineBranch',
          fix(fixer) { /* Report on the root node and rebuild the entire chain */
            const indent = ' '.repeat(node.loc?.start.column ?? 0)
            const lines: string[] = []
            ...
            for (let i = 0; i < conditions.length; i++) {
              ...
              if (branchSimplifiable[i]) {
                const stmt = getSingleStatement(consequent)! /* Simplify this branch */
                let stmtText = sourceCode.getText(stmt)
                if (!stmtText.endsWith(';') && !stmtText.endsWith('}')) stmtText = stmtText.trimEnd()
                lines.push(`${prefix} (${condText}) ${stmtText}`)
              } else {
