...
const rule: Rule.RuleModule = {
  meta: {
    ...
    return {
      ...
      //
      //   // 检查块是否可以压缩（单行简单语句）
      //   const canCompactTry = canBlockBeCompact(block)
      //   const canCompactCatch = handler && canBlockBeCompact(handler.body)
      //   const canCompactFinally = finalizer && canBlockBeCompact(finalizer)
      //
      //   // 只有当所有块都能压缩成单行时才触发修复（避免与 brace-style 冲突）
      //   if (!canCompactTry) return
      //   if (handler && !canCompactCatch) return
      //   if (finalizer && !canCompactFinally) return
      //
      //   // 检查块是否已经是压缩格式
      //   const isTryCompact = isBlockAlreadyCompact(block)
      //   const isCatchCompact = handler && isBlockAlreadyCompact(handler.body)
      //   const isFinallyCompact = finalizer && isBlockAlreadyCompact(finalizer)
      //
      //   // 检查 } catch / } finally 是否在同一行（1tbs 风格）
      ...
      //
      //   // 需要压缩的情况
      //   const needCompactTry = !isTryCompact
      //   const needCompactCatch = handler && !isCatchCompact
      //   const needCompactFinally = finalizer && !isFinallyCompact
      //   const needFixBraceStyle = (handler && !tryCatchSameLine) ?? (finalizer && !catchFinallySameLine && !tryFinallySameLine)
      //
      //   // 如果没有任何需要修复的，跳过
      //   if (!needCompactTry && !needCompactCatch && !needCompactFinally && !needFixBraceStyle) return
      //
      //   // 检查总长度是否超过限制
      //   const tryLen = getTryLineLength(block)
      //   const catchLen = handler ? getCatchLineLength(handler) : 0
      //   const finallyLen = finalizer ? getFinallyLineLength(finalizer) : 0
