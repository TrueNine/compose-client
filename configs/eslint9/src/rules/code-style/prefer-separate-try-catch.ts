  ...
  create(context) {
    ...
    return {
      TryStatement(node) {
        ...
        if (tokenBefore?.loc!.end.line === tryToken.loc.start.line) {
          context.report({node: tryToken, messageId: 'separateTry', fix: fixer => fixer.insertTextBefore(tryToken, '\n')})
        }
        ...
        if (node.handler) {
          ...
          if (tryCloseBrace.loc.end.line === catchToken!.loc.start.line) {
            context.report({node: catchToken!, messageId: 'separateCatch', fix: fixer => fixer.insertTextBefore(catchToken!, '\n')})
          }
        }
        ...
        if (prevCloseBrace.loc.end.line === finallyToken.loc.start.line) {
          context.report({node: finallyToken, messageId: 'separateFinally', fix: fixer => fixer.insertTextBefore(finallyToken, '\n')})
        }
      },
    }
  },
