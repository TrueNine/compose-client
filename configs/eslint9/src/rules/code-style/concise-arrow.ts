...
const rule: Rule.RuleModule = {
  ...
  create(context) {
          ...
          if (value !== null && typeof value === 'object') {
            if (Array.isArray(value)) {
              for (const item of value) {
                if (item !== null && typeof item === 'object' && 'type' in item) traverse(item as Rule.Node)
              }
            } else if ('type' in value) traverse(value as Rule.Node)
          }
        }
      }
    ...
    function needsParentheses(params: Rule.Node[]): boolean {
      ...
      if (paramWithType.typeAnnotation !== void 0 && paramWithType.typeAnnotation !== null) return true
      return false
    }
