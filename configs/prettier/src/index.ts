export default {
  ...
  trailingComma: 'none', // 是否多行中输入尾逗号 none:无 es5:添加 es5 中支持的尾逗号 all:所有可能的地方添加尾逗号
  endOfLine: 'lf', // 换行符
  arrowParens: 'avoid', // 是否为箭头函数参数添加括号 avoid: x => x always: (x) => x
  insertPragma: false, // 是否允许 Prettier 为已经格式化过的文件标记
  vueIndentScriptAndStyle: false // 是否在 vue 文件中的 models 和 style 标签内缩进代码
}
