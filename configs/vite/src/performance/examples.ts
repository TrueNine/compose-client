...
const MONOREPO_PACKAGE_PATTERN = /^@truenine\// // Static regex patterns for monorepo package detection
const API_PROXY_REWRITE_PATTERN = /^\/api/
...
export function createAdvancedDevelopmentExample(): UserConfig {
  ...
  const devConfig = createSmartDevelopmentOptimization({
    ...
    open: true,
    https: false,
    proxy: {'/api': {target: 'http://localhost:8080', changeOrigin: true, rewrite: (path: string) => path.replace(API_PROXY_REWRITE_PATTERN, '')}}
  })
